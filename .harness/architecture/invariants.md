...
3. 远端响应页数必须与本地输入页数一致。
4. PaddleX 主链路及归一化采用 fail-fast：任一失败导致整份文档失败，不写伪成功结果；可选 VLM 增强失败按其独立软降级规则处理。
...
1. VLM 默认关闭，只处理 PaddleX 已检测到且印章 OCR 文字为空、缺少置信度或低于配置阈值的印章；不审核整页漏检。
2. VLM 只能修改印章可转写文字，不得修改类型、坐标、顺序、`seal_id` 或引用目标。
3. 原始裁剪图和红色隔离视图必须各自返回完整可读且规范化后完全一致的文字；任一不可读、截断、结构异常或结果不一致均拒绝采用。
4. VLM 超时、传输失败、服务错误、图片损坏或校验失败时保留 PaddleX 内容，不得使主解析链路失败。
5. 图片、Base64、提示词、完整远端响应和正文不得写入日志或异常；裁剪图只在内存中短暂存在。
6. VLM 未启用或候选不触发时输出保持既有结构；触发后的审计信息只能作为额外可选字段出现。
